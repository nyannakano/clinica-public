...
class PagamentosController extends Controller
{
    ...
    {
        $pagamentos = Pagamento::all()->where('pag_type', '=', 0)->paginate(10);
    ...
    public function viewContaAReceber($idpagamento)
    {
        ...
        return view('pagamentos.viewcontaareceber', compact('pagamento',
    ...
    public function generate($idpagamento)
    {
        ...
        for ($i; $i <= $quantidade + $parcelas->count(); $i++) {
            Parcela::create([
                ...
                'par_type' => 0, // esse 0 é wereferente a conta a receber, em breve fará mais sentido pois terá contas a pagar também
                'par_status' => 0
            ]);
        }

        return redirect()->route('pagamentos.viewcontaareceber', ['id' => $idpagamento]);
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('pagamentos.viewcontaareceber', ['id' => $parcela->pag_id]);
    }
    ...
    public function receive(Request $request)
    {
        ...
        if (!$cliente == null) {
            ...
            $description = "Parcela número: " . $parcela->par_number . "; Conta a Receber número: " . $pagamento->id;
            $clienteid = null;
        }
    ...
    public function create()
    {
        ...
        return view('pagamentos.create', compact('contas', 'meios', 'clientes', 'profissionais'));
    }
    ...
    {
        $pagamentos = Pagamento::all()->where('pag_type', '=', 0)
    ...
    public function halfpay($valortotal, $valorpago, $pagamentoid, $number)
    {
        ...
        Parcela::create([
            ...
            'pag_id' => $pagamento->id,
            'par_deadline' => Carbon::now()->toDate(),
            'par_type' => 0, // esse 0 é referente a conta a receber
            'par_status' => 0
        ]);
    }
    ...
    public function filterClient($id)
    {
        ...
        $pagamentos = Pagamento::all()->where('pag_type', '=', 0)
            ->where('clie_id', '=', $cliente->id)->paginate(10);
        $ordens = OrdemDeServico::all();
        $profissionais = Profissional::all();


        return view('pagamentos.filterclient',
            compact('pagamentos', 'ordens',
