...
@section('content')
    ...
    @if($pagamento->pag_open == 2)
        <div class="alert alert-danger" role="alert">
            Esta conta a pagar está cancelada, não é possível modificá-la.
        </div>
    @elseif($pagamento->pag_open == 1)
        <div class="alert alert-info" role="alert">
            Esta conta a pagar está encerrada, não é possível modificá-la.
        </div>
    @endif
        ...
        @if($parcelascount < $pagamento->pag_indicator)
            <div class="d-flex bd-highlight mb-3">
                <div class="mr-auto p-2 bd-highlight">
                    <form action="{{ route('pagar.generate', ['id' => $pagamento->id]) }}" method="post">
                        @csrf
                ...
                <td>
                    @if($parcela->par_status == 0)
                        ...
                        <div class="modal fade" id="parcela{{ $parcela->id }}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form action="/contaapagar/editar/{{ $parcela->id }}"
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="input-group mb-2">
                                                <input type="number" step="0.01" min="0.01" required
                                                       class="form-control"
                                                       name="par_price"
                                                       value="{{ $parcela->par_price }}" placeholder="Valor da Parcela">
                                            </div>
                        ...
                        <div class="modal fade" id="lancar{{ $parcela->id }}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title" id="areaLabel">Lançar Pagamento
                                            {{ $parcela->par_number }}</h4>
                                    </div>
                                    <form action="/contaapagar/pagar/{{ $parcela->id }}"
                                          enctype="multipart/form-data"
                                          method="post">
                                        @csrf
                                        <div class="modal-body">
                                            ...
                                            <label for="par_price" class="tituloCampo">Juros/Multa/Valor
                                                Adicional:</label>
                                            <div class="input-group mb-2">
                                                <input type="number" step="0.01" min="0.0" required class="form-control"
                                                       name="par_additional"
                                                       value="0.0">
                                            </div>
                                            ...
                                            <label for="par_date" class="tituloCampo">Data de Recebimento:</label>
                                            <div class="input-group mb-2">
                                                <input type="date" required class="form-control"
                    ...
                    @if($pagamento->pag_open == 0)
                        ...
                        <div class="btn-group" role="group">
                            <form action="/contaapagar/pagar/remover/{{ $parcela->id }}" method="post">
                                @csrf
                                @method('DELETE')
        ...
        <tr class="thead-dark">
            <th>
                @if($pagamento->pag_open == 0 || $pagamento->pag_open == 1)
                    <form action="/contaapagar/pagar/cancelar/{{ $pagamento->id }}" method="post">
                        @csrf
                        <button name="remover" id="remover" class="btn btn-danger btn-secondary"
                                onclick="return confirm('Deseja realmente cancelar esta conta a pagar?')">
                            Cancelar
                        </button>
                    </form>
                @endif
